...
Sediment deposited by streams is called **alluvium** and includes channel deposits, floodplain deposits, or deposits near the mouth.

## Channel deposits

Alluvium deposited within a stream's channel is mostly sand and gravel **bars**.

**Natural levees** are masses of sand and gravel deposited on floodplains that form a slope away from the stream channel, since when it floods more alluvium is deposited close to the stream on the floodplain than far away, creating this slope.

## Graded Stream

A stream that has attained a balance of energy required to transport its load and experiences no net erosion or deposition.

Time lets the stream approach equilibrium with its new conditions.

The **base level** is the lowest elevation to which a stream can erode its bed. Base level most commonly corresponds to the elevation at which a stream enters another water body, where, on escaping the confinement of its channel, it loses velocity and loses its ability to erode.

Sea level is the ultimate base level.

**Braided Stream** = stream deposits materials on the floor of its channel, and these can prevent further water from passing, causing the stream to fork and rejoin in a network of channels that we called braided together.

**Meandering Stream** = Streams that occupy broad floodplains, whether erosional or depositional, have a single channel with sweeping meanders.

## Alluvial Fans and Deltas

Two of the most common landforms composed of alluvium are alluvial fans and deltas. They are deposited as a result of a loss of competence of a stream.

Alluvial fans are deposited on land, whereas deltas are deposited in a body of water.

Alluvial fans can be steep, whereas deltas are usually pretty flat.
...
Alluvial fans form in response to the abrupt drop in gradient combined with the change from a narrow channel of a mountain stream to the less confined channels at the base of a mountain.

The sudden drop in velocity causes the stream to dump its load of sediment quickly in a cone or fan-shaped accumulation.

### Deltas

A delta forms when a stream enters an ocean or a lake. As the stream's flow is checked on entering the lake, the dying current deposits its load of sediments in three types of beds:

1. **Foreset beds** composed of coarser particles that drop almost immediately on entering the lake to form layers that slope downcurrent on the *delta front*.

2. **Topset beds** that cover foreset beds, dominated by mud and plant debris on the upper surface of the delta.

3. **Bottomset beds** that consistent of the finer silts and clays that settle some distance from the mouth on an area called the *prodelta*.

As the delta grows outward, the channel becomes choked with sediment from the slowing water. The water then divides from the main channel into smaller ones, called **distributaries**.

Rather than **tributaries**, which carry water into the main channel, **distributaries** carry water away from the main channel in varying paths.

## Drainage Patterns

The **drainage basin** is the land area that contributes water to the stream. The drainage basin of one stream is separated from another by an elongated topographic high called a **divide**.

Divides range in size from a ridge separating two gullies to continental basins.

The Mississipi river has the largest drainage basin in North America.

**Dendritic Pattern** = most common drainage pattern. Characterized by irregular branching of tributaries that resembles the branches of a tree.

**Radial Pattern** = streams that diverge from a central area, like spokes from the hub of a wheel.

**Rectangular Pattern** = many right-angle bends, where the bedrock is criss-crossed by joints and faults.

## Headward Erosion and Stream Piracy

A stream can lengthen its course by **headwater erosion**, extending from the head of its valley upslope. **Stream piracy** is the diversion of the drainage of one stream because of the headward erosion of another stream.

If a stream on one side of a divide has a steeper gradient and more energy than a stream on the other side, it can extend its valley headward, eventually breaking down the divide and capturing part or all of the drainage of the slower stream.

## Floods and Flood Control

The **recurrence interval** is an estimate of how often a flood of a given size can be expected to occur.

## Causes and Types of Floods

### Regional Floods

Rapid melting of snow in spring and heavy rain can overwhelm a river. Regional floods are commonly caused by slow-moving storm systems, like decaying hurricanes.

## Flash Floods

Factors influencing flash flooding include rainfall intensity and duration, surface conditions and topography.

Mountainous areas are susceptible because steep slopes can funnel runoff into narrow canyons.

Urban areas are also at risk because a high percentage of the surface area is composed of impervious roofs, streets and parking lots where runoff is rapid.

Streets, etc cover the ground that once soaked up water. Less water infiltrates and the rate and amount of runoff increase.

## Ice-Jam Floods

As the level of a stream rises, it breaks up the ice and creates ice flows that can pile up to make channel obstructions, or an ice dam. Water upstream from the ice can rise and overflow the channel banks.

When the ice dam fails, the water stored behind the dam is released, causing a flash flood downstream.

## Dam-Failure Flood

Dams and artificial levees are built for flood protection of a certain magnitude, if the dam or levee fails, a flash flood occurs.
...
Built to store floodwater and then let it out slowly. Lowers the flood crest by spreading it out over a longer time.

### Channellization

Involves alteration of a stream channel to speed the flow of water and prevent it from reaching flood height. Can mean clearing a channel of obstructions, or dredging a channel to make it wider or deeper.

Can create artificial cutoffs that shorten and straighten the stream, increasing the gradient, velocity, discharge and erosion.
